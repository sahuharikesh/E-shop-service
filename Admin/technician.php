...
<div class="col-sm-9 col-md-10 text-center ">
    <p class="blkColor text-white  mt-5 p-2">List of Technician</p>
    <?php   $sql = "SELECT * FROM `technician`";
                    ...
                    if($result->num_rows > 0){
                        echo'<table class="table mt-3 mr-5">
                             ...
                             echo '<input type="hidden" name="id" value='.$row["tid"].'><button class="btn " 
                             name ="edit" value ="Edit" type="submit"><i class="fas fa-pen"></i></button>';
                             echo '</form>';
                             echo '<form action="" method="post" class="d-inline mr-2">';
                             echo '<input type="hidden" name="id" value='.$row["tid"].'><button class="btn btn1" 
                    ...
                    if(isset($_REQUEST['delete'])){
                        $sql = "DELETE FROM `technician` WHERE tid = {$_REQUEST['id']}";
...
<div class="float-right"><a href="techAdd.php" class="btn  mx-3 mb-5"><i class="fa fa-plus "></i></a></div>
 </div>
